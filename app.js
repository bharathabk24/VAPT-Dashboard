  ...
  async insertBatch(table, rows) {
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      ...
      const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, {
        method: 'POST',
        headers: { ...this.hdr, 'Prefer': 'resolution=ignore-duplicates' },
...
async function loadFromSupabase(){
  showLoading('Loading vulnerability data from database…');
  try {
    let vulnRows=[], from=0;
    const step=10000;
    while(true){
      const chunk=await sb.select('vulnerabilities',
        `select=upload_date,vulnerability_name,severity,computer_name,operating_system,remote_office,cvss_score,discovered_date&order=upload_date.asc&limit=${step}&offset=${from}`);
      vulnRows=vulnRows.concat(chunk);
      if(chunk.length<step) break;
      from+=step;
      showLoading(`Loading… (${vulnRows.length.toLocaleString()} rows so far)`);
    }
    ...
    from=0;
    while(true){
      const chunk=await sb.select('resolutions',
        `select=computer_name,vulnerability_name,status,date,office,notes&order=date.asc&limit=${step}&offset=${from}`);
      resRows=resRows.concat(chunk);
      if(chunk.length<step) break;
      from+=step;
    }
    ...
    if(days.length){
      ...
      document.getElementById('fileInfo').textContent=
        days.length+' day(s) in database  |  Latest: '+days[days.length-1];
    } else {
    ...
    document.getElementById('fileInfo').textContent='⚠ DB error: '+err.message;
    console.error(err);
  }
  hideLoading();
}
...
// ═══════════════════════════════════════════════════════════════
//  UPLOAD MODAL — with batch date picker
// ═══════════════════════════════════════════════════════════════
...
document.getElementById('vulnFileInput').addEventListener('change',e=>{
  ...
  document.getElementById('vulnFileNames').textContent=pendingVulnFiles.map(f=>f.name).join(', ');
  const detected=dateFromFilename(pendingVulnFiles[0]?.name||'');
  if(detected) document.getElementById('batchDateInput').value=detected;
  e.target.value='';
});
document.getElementById('resFileInput').addEventListener('change',e=>{
  pendingResFiles=Array.from(e.target.files);
  document.getElementById('resFileNames').textContent=pendingResFiles.map(f=>f.name).join(', ');
  e.target.value='';
});
...
async function startUpload(){
  ...
  if(!batchDate){ alert('Please select a date for this batch.'); return; }
  ...
  const total=pendingVulnFiles.length+pendingResFiles.length;
  let done=0;

  // Parse all vuln files and group by date
  const byDate={};
  for(const file of pendingVulnFiles){
    try{
      logProgress('📄 Parsing: '+file.name);
      const rows=await parseFile(file);
      // Use date from filename if available, otherwise use the batch date picker
    ...
    done++; setProgressBar(Math.round(done/total*70));
  }

  // Upload each date group (merges multiple files for same day)
  for(const [date,rows] of Object.entries(byDate)){
    try{
      logProgress('\n💾 Saving '+rows.length.toLocaleString()+' rows for '+date+'…');
      await sb.insertBatch('vulnerabilities', rows.map(r=>vulnRowToDb(r,date)));
      logProgress('✅ '+date+' saved');
    } catch(err){
  ...
  for(const file of pendingResFiles){
    try{
      ...
      const rows=await parseFile(file);
      logProgress('💾 Saving '+rows.length.toLocaleString()+' resolution rows…');
      await sb.insertBatch('resolutions', rows.map(r=>resRowToDb(normaliseResRow(r))));
      logProgress('✅ '+file.name+' saved');
    } catch(err){
      logProgress('❌ Error '+file.name+': '+err.message);
    }
    done++; setProgressBar(Math.round(done/total*100));
  }

  setProgressBar(100);
  logProgress('\n🎉 All done! Reloading dashboard…');
  setTimeout(async()=>{ closeUploadModal(); await loadFromSupabase(); },1800);
}
...
function rebuildDayDropdowns(){
  ...
  document.getElementById('diff-day-a').innerHTML='<option value="">— select Day A —</option>'+opts;
  document.getElementById('diff-day-b').innerHTML='<option value="">— select Day B —</option>'+opts;
  if(days.length>=2){
    document.getElementById('diff-day-a').value=days[days.length-2];
    document.getElementById('diff-day-b').value=days[days.length-1];
  }
  const allOff=[...new Set(Object.values(dayStore).flat().map(r=>r['Remote Office']).filter(Boolean))].sort();
  document.getElementById('trend-office-filter').innerHTML=
    '<option value="">All offices</option>'+allOff.map(o=>`<option value="${o}">${o}</option>`).join('');
  updateDayNavLabel();
}
...
function switchDay(dateKey){
  ...
  document.getElementById('last-updated').textContent=
    'Viewing: '+dateKey+'  ('+fmt((dayStore[dateKey]||[]).length)+' records)';
  updateDayNavLabel();
  populateGlobalDropdowns();
  applyFilters();
}
...
function populateGlobalDropdowns(){
  ...
  ['f-office-global','f-office'].forEach(id=>{
    ...
    document.getElementById(id).innerHTML='<option value="">All offices</option>'+
      offices.map(o=>`<option value="${o}">${o}</option>`).join('');
    if(offices.includes(prev)) document.getElementById(id).value=prev;
  });
  document.getElementById('f-os-global').innerHTML='<option value="">All OS</option>'+
...
function applyFilters(){
  ...
  renderMetrics(filteredData.length,sev,devices,prevRows?countBy(prevRows,'Severity'):null,prevRows?prevRows.length:null);
  renderSevChart(sev);
...
function renderOfficeChart(officeCounts){
  const top=topN(officeCounts,8),labels=top.map(([k])=>k.length>22?k.slice(0,20)+'…':k),data=top.map(([,v])=>v);
...
function renderOSBars(osCounts){
  const top=topN(osCounts,10),max=top[0]?top[0][1]:1;
...
function rebuildDiff(){
  ...
  if(!dayA||!dayB||dayA===dayB){
    document.getElementById('diff-summary').innerHTML=
      `<div style="color:var(--text-3);font-size:13px;padding:20px 0;">
        Select two different days to compare.
        <br><span style="font-size:11px;opacity:0.6;">You currently have ${sortedDays().length} day(s) in the database.</span>
      </div>`;
    return;
  }
  ...
  const cards=[
    ...
    {label:'Fixed / Resolved',val:fmt(resolvedRows.length),color:'var(--green)'},
...
function renderResolution(){
  ...
  document.getElementById('res-subtitle').textContent  =fmt(total)+' records  |  '+resRate+'% overall success rate';
  ...
  const resVals=resLabels.map(k=>sc[k]||0);
  ...
  // Office resolution breakdown
  const byOffice={};
  resData.forEach(r=>{
    const o=r['Office']||'Unknown';
    if(!byOffice[o]) byOffice[o]={succeeded:0,failed:0,pending:0,total:0};
    byOffice[o].total++;
    const s=r['Status']||'';
    if(s==='Succeeded') byOffice[o].succeeded++;
    else if(s==='Failed') byOffice[o].failed++;
    else byOffice[o].pending++;
  });
  const topOff=Object.entries(byOffice).sort((a,b)=>b[1].total-a[1].total).slice(0,10);
...
function rebuildTrend(){
  ...
  mkChart('trendTotalChart','trendTotalChart',{type:'line',
    data:{labels:days,datasets:[{label:'Total vulns',data:totals,
  ...
  mkChart('trendResChart','trendResChart',{type:'line',
    data:{labels:days,datasets:[{label:'Resolution success %',data:rateData,
